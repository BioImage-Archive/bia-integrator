...
from bia_ingest.persistence_strategy import persistence_strategy_factory, PersistenceMode
...
def process_exported_json(exported_json_path: str):
    ...
    for eci in exported_json.values():
        ...
        for representation in eci["representation"]:
            if len(representation["file_uri"]) > 0 and representation["file_uri"][0].find(accession_id_to_process) > 0:
        ...
        file_reference_uuid = representation["original_file_reference_uuid"]
        assert len(file_reference_uuid) == 1
        # Create Image
        image_uuid = [uuid_creation.create_image_uuid(file_reference_uuid),]
        image = None
        file_reference_uuid = str(file_reference_uuid[0])
        ...
        try:
            ...
            if result.returncode != 0:
                logger.error(f"Problem creating Image with UUID {image_uuid[0]} for {accession_id}. Error was {result.stderr} - abandoning migration for representations linked to this Image!!!")
                continue
            image = persister.fetch_by_uuid(image_uuid, bia_data_model.Image)
        assert image
        ...
        for old_representation in representations_to_process:
            ...
            logger.info(f"Creating new representation for old representation with uuid {old_representation['uuid']}.")
            result = subprocess.run([
                    "poetry",
                    "--directory",
                    poetry_base_dir,
            ...
            if result.returncode != 0:
                logger.error(f"Problem creating ImageRepresentation for old image with UUID {old_representation['uuid']} for {accession_id}. Error was {result.stderr} - abandoning migration for this representation!!!")
                continue
            ...
            new_representation_uuid = uuid_creation.create_image_representation_uuid(image_uuid[0], old_representation["image_format"], use_type)
            new_representation = persister.fetch_by_uuid([new_representation_uuid,], bia_data_model.ImageRepresentation)[0]
            if use_type in (ImageRepresentationUseType.STATIC_DISPLAY.value, ImageRepresentationUseType.THUMBNAIL.value):
                ...
                new_representation.total_size_in_bytes = old_representation["total_size_in_bytes"]
                new_representation.physical_size_x = old_representation["physical_size_x"]
                new_representation.physical_size_y = old_representation["physical_size_y"]
                new_representation.physical_size_z = old_representation["physical_size_z"]
                ...
                if old_representation["attribute"]:
                    ...
                    new_representation.attribute = [semantic_models.Attribute.model_validate(new_attribute_dict),]
            else:
                logger.error(f"Unknown use type: {use_type} Not updating ImageRepresentation with UUID {new_representation.uuid}")
                continue

            # Save updated representation
            persister.persist([new_representation,])
            logger.info(f"Saved updated ImageRepresentation with UUID {new_representation.uuid}")
