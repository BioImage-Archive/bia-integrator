...
class ImageDependencies(BaseModel):
    ...
    def has_dependencies_for_creation_process_creation(self) -> bool:
        if (
            not self.has_typical_annotation_image_leaf_objects
...
def find_exisiting_image_dependencies(file_references, api_client):
...
def create_missing_dependencies(
    ...
    assert existing_dependencies.has_dependencies_for_creation_process_creation()
