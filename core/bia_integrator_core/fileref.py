...
def get_filerefs(study_accession_id: str, limit: int = 10**6) -> List[api_models.FileReference]:
    ...
    filerefs = settings.api_client.get_study_file_references(study_obj_info.uuid, limit=limit)

    return filerefs

def get_fileref(fileref_uuid: str) -> api_models.FileReference:
    """Return all filerefs stored on disk for the given accession."""
    
    fileref = settings.api_client.get_file_reference(fileref_uuid)
