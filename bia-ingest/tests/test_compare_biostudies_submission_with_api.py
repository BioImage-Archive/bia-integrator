...
def setup_module(module):
    ...
    rw_client.get_study = MagicMock(
        return_value = expected_study_api_model
    )

    mock_object_info = Mock(uuid=expected_study_api_model.uuid)
    rw_client.get_object_info_by_accession = MagicMock(
        return_value = [mock_object_info,]
    )
...
def test_compare_biostudies_with_api_function():
    ...
    comparison_result = compare_biostudies_with_api(flattened_contents_biostudies, flattened_contents_api)
