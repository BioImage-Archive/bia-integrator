...
def compare_biostudies_with_api(from_biostudies: dict[str, Any], from_api: dict[str, Any]) -> dict[str, list[str]]:
...
def main(accession_id: str, output_path: str = None) -> None:
    ...
    if n_studies == 0:
        ...
        study_dict["file_references"] = [fileref.dict for fileref in rw_client.get_study_file_references(study.uuid)]
        study_dict["images"] = [image.dict for image in rw_client.get_study_images(study.uuid)]
        visitor.reset(accession_id)
        visitor.visit("", study_dict)
        from_api = visitor.flattened_contents_dict
    else:
        raise Exception(f"Got more than one value querying info for {accession_id}. Return value: {study_info}")
