...
def update_channel_names_zattrs(zattrs_path: Path, channel_names: list):
    ...
    if not zattrs_path.is_file():
        logger.error(
            f"Could not find {zattrs_path}. Channel details not updated in .zattrs"
        )
        return
    ...
    try:
        ...
        logger.error(
            f"An error occured. Channel details not updated in .zattrs. Error was {err}"
        )
        return

    if n_zarr_channels != n_channels:
        logger.error(
            f"Expected {n_channels} channels, got {n_zarr_channels}: Channel details not updated in {zattrs_path}"
        )
        return
...
def update_channel_names_xml(xml_path: Path, channel_names: list):
    ...
    xml_channels = [
        descendant for descendant in root.iter() if descendant.tag.endswith("Channel")
    ]

    n_xml_channels = len(xml_channels)
    if n_xml_channels != n_channels:
        logger.error(
            f"Expected {n_channels} channels, got {n_xml_channels}: Channel details not updated in {xml_path}"
        )
        return
...
def convert_multichannel_to_zarr_and_upload(
    accession_id, image_id, src_uris, pattern=None, channel_names=None
):
    ...
    if not zarr_fpath.exists():
        ...
        if channel_names is not None and len(channel_names) > 0:
            update_channel_names_xml(
                zarr_fpath / "OME" / "METADATA.ome.xml", channel_names
            )
...
def main(yaml_fpath):
    ...
    for image_description in raw_config["images"].values():
        ...
        image_rep = BIAImageRepresentation(
            ...
            attributes={"fileref_ids": fileref_ids, "structure": structure},
