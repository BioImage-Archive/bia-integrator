...
def set_multichannel_rendering(zmeta):
    ...
    zmeta.omero.channels[3].color = "00FFFF"

    # zmeta.omero.channels[4].active = True
    # zmeta.omero.channels[4].color = "FFFF00"
...
def main(accession_id, image_id):
    ...
    # if len(zmeta.omero.channels) == 1:
    #     zmeta.omero.channels[0].color = "FFFFFF"
    # else:
    #     set_multichannel_rendering(zmeta)
    ...
    for multiscale in zmeta.multiscales:
        for axis in multiscale.axes:
            if not axis.unit:
                default_val = default_unit_map[axis.name]
                # print(f"Nothing for {axis.name}, should use {default_val}")
                axis.unit = default_val
