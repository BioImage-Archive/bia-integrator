...
def main(input_image_fpath, accession_id):
    ...
    dst_key = f"{accession_id}-annotation-example.png"
    example_annotation_uri = copy_local_to_s3(input_image_fpath, dst_key)

    logger.info(f"Uploaded as {example_annotation_uri}")

    annotation = StudyAnnotation(
        accession_id=accession_id,
        key="example_annotation_uri",
        value=example_annotation_uri
    )
