...
def image_representation_from_zarr(
    ...
    # TODO: this should be replace by API client and we would not need
    # accession_id
    file_references = [
...
def _get_file_reference_from_disk(
    uuid: UUID, accession_id: str
) -> bia_data_model.FileReference:
    """Get FileRefence from expected path on disk"""

    input_path = (
        Path(settings.bia_data_dir) / "file_references" / accession_id / f"{uuid}.json"
    )
    return bia_data_model.FileReference.model_validate_json(input_path.read_text())
