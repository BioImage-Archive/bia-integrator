...
jq -c 'to_entries | map(.value)[:1000000][] | ({"index": {"_index": "'"${ELASTIC_INDEX}"'"}}, .)' $EXPORT_JSON_OUT_FILE | sed > ${EXPORT_JSON_OUT_FILE}.bulk
...
jq -c 'to_entries | map(.value)[:1000000][] | ({"index": {"_index": "'"${ELASTIC_INDEX_IMAGES}"'"}}, .)' $EXPORT_JSON_OUT_FILE | sed > ${EXPORT_JSON_OUT_FILE}.bulk
