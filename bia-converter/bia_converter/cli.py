...
def convert(
    ...
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="If set, do not actually upload to s3")
    ] = False,
):
...
def update_recommended_vizarr_representation(
    ...
    api_client = get_api_client(api_target)
    image_rep = api_client.get_image_representation(image_rep_uuid)
    convert_module.update_recommended_vizarr_representation_for_image(
...
def create_thumbnail(
    ...
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="If set, do not actually upload thumbnail to s3")
    ] = False,
):
    logging.basicConfig(level=logging.INFO)
...
def create_static_display(
    ...
    dry_run: Annotated[
        bool, typer.Option("--dry-run", help="If set, do not actually upload image to s3")
    ] = False,
):
    logging.basicConfig(level=logging.INFO)
