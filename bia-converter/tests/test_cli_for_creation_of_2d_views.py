...
@pytest.fixture
def expected_thumbnail_uri(input_image_uuid) -> str:
    return [f"{settings.endpoint_url}/{settings.bucket_name}/{accession_id}/{input_image_uuid}/thumbnail_256x256.png",]


@pytest.fixture
def expected_static_display_uri(input_image_uuid) -> str:
    return [f"{settings.endpoint_url}/{settings.bucket_name}/{accession_id}/{input_image_uuid}/static_display_512x512.png",]


def setup_input_image_and_return_uuid() -> str:
    input_image_path = Path(__file__).parent / "data" / "input_image_for_creating_2d_views.json"
...
def setup_interactive_image_rep_and_return_uuid() -> str:
    ...
    try:
        interactive_image_rep = api_client.get_image_representation(interactive_image_rep_uuid)
    except NotFoundException:
        interactive_image_rep_path = Path(__file__).parent / "data" / "interactive_image_representation_for_creating_2d_views.json"
        interactive_image_rep_dict = json.loads(interactive_image_rep_path.read_text())
        interactive_image_rep = api_models.ImageRepresentation.model_validate(interactive_image_rep_dict)
        assert interactive_image_rep_uuid == interactive_image_rep.uuid

        # Ensure the file_uri points to the ome.zarr in tests/data
        file_uri_path = Path(__file__).parent.absolute() / "data" / "im07.ome.zarr" / "0"
...
def test_cli_convert_interactive_display_to_static_display(
    ...
    attributes = attributes_by_name(updated_image)
    key = "image_static_display_uri"
    assert key in attributes
    assert attributes[key][key] == expected_static_display_uri
    created_static_display_path = (
        settings.cache_root_dirpath / "mock_s3" / "static_display_512x512.png"
    )
...
def test_cli_convert_interactive_display_to_thumbnail(
    ...
    updated_image = api_client.get_image(input_image_uuid)
    attributes = attributes_by_name(updated_image)
    assert "image_thumbnail_uri" in attributes
    assert attributes["image_thumbnail_uri"]["image_thumbnail_uri"] == expected_thumbnail_uri
    created_thumbnail_path = (
        settings.cache_root_dirpath / "mock_s3" / "thumbnail_256x256.png"
    )
