...
def get_test_filerefs(locust_user: APIUserBase, n_filerefs):
    ...
    rsp = locust_user.client.get(
        f"api/studies/{study_uuid}/file_references?limit={n_filerefs}"
    )
    ...
    ), f"Study {study_uuid} has {len(filerefs)} file references, but {n_filerefs} were requested."
        ...
        images.append(
            {
                ...
                "alias": {"name": img_alias_from_uuid(uuid)},
            }
        )
    return images
...
def get_test_images(locust_user: APIUserBase, n_images):
    ...
    assert (
        len(images) == n_images
    ), f"Study {study_uuid} has {len(images)} file references, but {n_images} were requested."
