...
async def get_study_images_by_alias(
    ...
    if not len(study_objects_info):
        raise DocumentNotFound(
            f"Study with accession {study_accession} does not exist."
        )
...
async def search_images_exact_match(
    ...
    if search_filter.annotations_any:
        obj_annotations_any = [
            query_term.model_dump(exclude_defaults=True)
            for query_term in search_filter.annotations_any
        ]
    ...
    if search_filter.image_representations_any:
        ...
        for representation_filter in search_filter.image_representations_any:
            ...
            if any(
                [
                    representation_filter.size_bounds_gte is not None,
                    representation_filter.size_bounds_lte is not None,
                ]
            ):
                ...
                if representation_filter.size_bounds_gte is not None:
                    img_representation_list_item_filter["size"][
                        "$gte"
                    ] = representation_filter.size_bounds_gte
                if representation_filter.size_bounds_lte is not None:
                    img_representation_list_item_filter["size"][
                        "$lte"
                    ] = representation_filter.size_bounds_lte
    ...
    images = await db.search_images(
        query, start_uuid=search_filter.start_uuid, limit=search_filter.limit
    )
    annotator.annotate_if_needed(images)
...
async def search_file_references_exact_match(
    ...
    if search_filter.file_reference_match:
        ...
        if any(
            [
                search_filter.file_reference_match.size_bounds_gte is not None,
                search_filter.file_reference_match.size_bounds_lte is not None,
            ]
        ):
            query["size_in_bytes"] = {}
            if search_filter.file_reference_match.size_bounds_gte is not None:
                query["size_in_bytes"][
                    "$gte"
                ] = search_filter.file_reference_match.size_bounds_gte
            if search_filter.file_reference_match.size_bounds_lte is not None:
                query["size_in_bytes"][
                    "$lte"
                ] = search_filter.file_reference_match.size_bounds_lte

    if query == {}:
        raise InvalidRequestException("Expecting at least one filter when searching")

    file_references = await db.search_filerefs(
        query, start_uuid=search_filter.start_uuid, limit=search_filter.limit
    )
...
async def search_studies_exact_match(
    ...
    if search_filter.study_match:
        if any(
            [
                search_filter.study_match.file_references_count_gte is not None,
                search_filter.study_match.file_references_count_lte is not None,
            ]
        ):
            ...
            if search_filter.study_match.file_references_count_gte is not None:
                query["file_references_count"][
                    "$gte"
                ] = search_filter.study_match.file_references_count_gte
            if search_filter.study_match.file_references_count_lte is not None:
                query["file_references_count"][
                    "$lte"
                ] = search_filter.study_match.file_references_count_lte
        if any(
            [
                search_filter.study_match.images_count_gte is not None,
                search_filter.study_match.images_count_lte is not None,
            ]
        ):
            query["images_count"] = {}
            if search_filter.study_match.images_count_gte is not None:
                query["images_count"][
                    "$gte"
                ] = search_filter.study_match.images_count_gte
            if search_filter.study_match.images_count_lte is not None:
                query["images_count"][
                    "$lte"
                ] = search_filter.study_match.images_count_lte
        if search_filter.study_match.author_name_fragment:
            query["authors"] = {
                "$elemMatch": {
                    "name": {
                        "$regex": re.escape(
                            search_filter.study_match.author_name_fragment
                        ),
                        "$options": "i",
                    },
                },
    ...
    studies = await db.search_studies(
        query, start_uuid=search_filter.start_uuid, limit=search_filter.limit
    )
    annotator.annotate_if_needed(studies)
