...
def test_create_file_references_multiple_errors(
    api_client: TestClient, existing_study: dict, existing_file_reference: dict
):
    ...
    for write_result in bulk_write_results_by_status[400]:
        ...
        if "E11000 duplicate key error" in write_result["message"]:
            ...
            existing_item_shaped_as_rejected = {
                k: existing_item[k] for k in rejected_item.keys()
            }
    ...
    assert (
        f"{file_references[3]['study_uuid']} does not exist"
        in bulk_write_results_by_status[400][3]["message"]
    )
...
def test_create_file_references_existing_unchaged(
    ...
    assert_bulk_response_items_correct(
        api_client, file_references, create_result, f"file_references"
    )


def test_create_file_references_existing_changed(
    api_client: TestClient, existing_study: dict, existing_file_reference: dict
):
...
def test_create_file_references_missing_study(
    api_client: TestClient, existing_study: dict
):
    file_references = [
        get_template_file_reference(existing_study, add_uuid=True) for _ in range(2)
    ]
...
def test_create_file_references_nonzero_version(
    api_client: TestClient, existing_study: dict
):
    file_references = [
        get_template_file_reference(existing_study, add_uuid=True) for _ in range(2)
...
def test_create_file_references_same_request_duplicates(
    api_client: TestClient, existing_study: dict
):
...
def test_create_file_references_same_request_almost_duplicates(
    api_client: TestClient, existing_study: dict
):
    ...
    assert (
        api_client.get(f"file_references/{file_references[0]['uuid']}").status_code
        == 200
    )
...
def test_create_file_references_idempotent_on_identical_ops_when_defaults_missing(
    api_client: TestClient, existing_study: dict, existing_file_reference: dict
):
...
def test_update_file_reference(api_client: TestClient, existing_file_reference: dict):
    ...
    rsp = api_client.patch(
        "private/file_references/single", json=existing_file_reference
    )
    assert rsp.status_code == 200, rsp.json()


def test_file_reference_pagination_defaults(
    api_client: TestClient, existing_study: dict
):
...
def test_file_reference_pagination(api_client: TestClient, existing_study: dict):
    ...
    # 1,2
    rsp = api_client.get(
        f"studies/{existing_study['uuid']}/file_references?limit={chunk_size}"
    )
    assert rsp.status_code == 200
    file_references_fetched = rsp.json()
    ...
    # 3,4
    rsp = api_client.get(
        f"studies/{existing_study['uuid']}/file_references?start_uuid={file_references_fetched[-1]['uuid']}&limit={chunk_size}"
    )
    assert rsp.status_code == 200
    file_references_fetched = rsp.json()
...
def test_file_reference_large_page(api_client: TestClient, existing_study: dict):
    ...
    rsp = api_client.get(
        f"studies/{existing_study['uuid']}/file_references?limit={10000}"
    )
    assert rsp.status_code == 200
    file_references_fetched = rsp.json()
...
def test_file_reference_pagination_bad_limit(
    api_client: TestClient, existing_study: dict
):
    file_references = make_file_references(api_client, existing_study, 5)
    file_references.sort(key=lambda img: UUID(img["uuid"]).hex)
...
class TestSearchFilerefExactMatch:
    @pytest.fixture
    def fileref_fixtures(
        self, api_client: TestClient, existing_study: dict
    ) -> List[dict]:
        first_fileref = get_template_file_reference(
            existing_study=existing_study, add_uuid=True
        )
        first_fileref |= {
        ...
        second_fileref = get_template_file_reference(
            existing_study=existing_study, add_uuid=True
        )
        second_fileref |= {
            "uri": "https://google.com/second_fileref/placeholder",
            "size_in_bytes": 100,
            "attributes": {
                "some_attr": "some_value",
        ...
        dummy_fileref = get_template_file_reference(
            existing_study=existing_study, add_uuid=True
        )

        filerefs_created = [first_fileref, second_fileref, dummy_fileref]
        rsp = api_client.post("private/file_references", json=filerefs_created)
        assert rsp.status_code == 201, rsp.json()

        return filerefs_created
    ...
    def test_search_pagination(self, api_client: TestClient):
        ...
        filerefs_fetched = rsp.json()
        assert len(filerefs_fetched) == 2

        last_img_first_page = filerefs_fetched[-1]["uuid"]
        rsp = api_client.post(
            "search/file_references/exact_match",
            json={
                "file_reference_match": {
